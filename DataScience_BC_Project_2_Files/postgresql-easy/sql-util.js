class SqlUtil {
  ...
  /**
   * Gets a record from a given table by ticker.
   * This requires the table to have a column named "ticker".
   */
   getByTicker1(tableName /*, ticker*/) {
    const sql = `select * from ${tableName} where ticker=$1`;
    this.log('getByTicker1: sql =', sql);
    return sql;
  }
